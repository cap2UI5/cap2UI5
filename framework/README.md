...
|---|---|---|
| [`framework/`](.) (here) | transpiler + sync scripts → generates [`core/`](core/); hosts the [adapters](adapters/) | `npm install && npm test` |
...
upstream ──────────────▶ run/input/ ──────────────▶ run/output/ ──┐
 ...
 app/webapp              app/webapp        patch      app/         │
                                                                   │
framework/src/  (hand-written framework: engine + ports + class ports) ──┘
```
...
`framework/core/` + `cap2UI5/` from the sources plus all three `run/output/`
...
|---|---|---|
| **update_samples** (03:00) | samples mirror → samples transpile → build_cap<br>(`mirror_samples`, `transpile_samples`, `build_cap`) | `framework/core/srv/app/samples` from the abap2UI5/samples **whole cloud branch** |
| **update_backend** (04:00) | abap2UI5 mirror → abap2UI5 transpile → build_cap<br>(`mirror_abap2ui5`, `transpile_abap2ui5`, `build_cap`) | `framework/core/srv/z2ui5` from the abap2UI5 framework `src/` |
| **update_frontend** (05:00) | app mirror → app prepare → build_cap<br>(`mirror_app`, `prepare_app`, `build_cap`) | `framework/core/app/z2ui5/webapp` + `cap2UI5/app/z2ui5/webapp` from the abap2UI5 `app/webapp` |
...
touching `framework/core/` or `cap2UI5/` to
