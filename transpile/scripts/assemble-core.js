#!/usr/bin/env node
/**
 * assemble-core — build the core package under transpile/run/output/core by
 * overlaying the generated trees on top of the hand-maintained base/ source:
 *
 *   base/**                  →  run/output/core/**             (verbatim — the source)
...
// Local-only artifacts that may exist in base/ when it was used standalone —
...
if (!fs.existsSync(base)) {
  console.error("transpile/base not found — it is the hand-maintained source of the core package");
  process.exit(1);
}

fs.rmSync(dest, { recursive: true, force: true });
copyDir(base, dest);
console.log(`base → run/output/core (source skeleton copied)`);
...
if (broken) console.error(`WARNING: ${broken} transpiled file(s) skipped (parse/load error) — fix transpile/scripts/abap2js.js`);
