# transpile/ — the framework build project
...
hand-maintained source in [`base/`](base/) and the upstream
...
[`builder-cap/`](../builder-cap/).

> [!IMPORTANT]
> **`core/` is a build artifact — do not hand-edit it.** Every build wipes and
> rewrites it. The hand-written source lives in [`base/`](base/); edit there
...
|---|---|---|
| [`transpile/`](.) (here) | transpiler + sync scripts → generates [`core/`](core/); hosts the [adapters](adapter/) | `npm install && npm test` |
| [`builder-cap/`](../builder-cap/) | generates the full CAP app from its `base/` + the core webapp | `npm run build_cap` |
...
core package by overlaying the generated trees on the hand-written `base/` →
...
upstream ──────────────▶ run/input/ ──────────────▶ run/output/ ──┐
 ...
 app/webapp              app/webapp        patch      app/         │
                                                                   │
transpile/base/  (hand-written framework: engine + ports + class ports) ──┘
```
...
package. Downstream, [`builder-cap/`](../builder-cap/) assembles the full CAP
app from its own `base/` plus the webapp shipped in the published core.
...
| `npm run assemble_core` | `base/` + the `run/output/` trees → `run/output/core` (the complete package) |
...
| [`base/`](base/) | **Hand-written SOURCE of the core package** (see below). |
| [`core/`](core/) | **The published package `abap2UI5`** — generated, do not hand-edit. |
| [`adapter/`](adapter/) | The four platform adapters (cap, node, express, web) — hand-written consumers of `core/`. |
...
`base/` is the **source of truth** for everything hand-written in the
...
[`builder-cap/base/`](../builder-cap/base/) — together with this base it forms
a fully functional minimal CAP project runnable without any generated
overlays; see [builder-cap/README.md](../builder-cap/README.md).
...
- **`run/output/core`** — the assembled package (`base` + overlays).
...
`assemble-core.js` copies `base/` verbatim, then overlays the generated trees:
...
- **samples** (`run/output/samples` → `core/srv/app/samples`): flattened (every
  ...
  overwritten; the hand-written `samples/README.md` from `base/` stays.
- **frontend** (`run/output/app` → `core/app/z2ui5/webapp`): replaced 1:1. The
  CAP app's webapp copy is made downstream by `builder-cap` from this published
...
`npm run build_cap` in `builder-cap/` — it regenerates the *whole*
`transpile/core/` + `cap2UI5/` from the bases plus all three `run/output/`
...
|---|---|---|
| **update_samples** (03:00) | samples mirror → samples transpile → build_cap<br>(`mirror_samples`, `transpile_samples`, `build_cap`) | `transpile/core/srv/app/samples` from the abap2UI5/samples **whole cloud branch** |
| **update_backend** (04:00) | abap2UI5 mirror → abap2UI5 transpile → build_cap<br>(`mirror_abap2ui5`, `transpile_abap2ui5`, `build_cap`) | `transpile/core/srv/z2ui5` from the abap2UI5 framework `src/` |
| **update_frontend** (05:00) | app mirror → app prepare → build_cap<br>(`mirror_app`, `prepare_app`, `build_cap`) | `transpile/core/app/z2ui5/webapp` + `cap2UI5/app/z2ui5/webapp` from the abap2UI5 `app/webapp` |
...
touching `transpile/core/` or `cap2UI5/` to
