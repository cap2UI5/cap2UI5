...
[`builder/base/cap/`](https://github.com/cap2UI5/cap2UI5/tree/main/builder/base/cap)
...
the platform-neutral **core package** (`core/`, linked as the npm dependency
`abap2UI5` via `file:../core`) and is generated by the sync pipeline. All
commands below work in both places: in `builder/base/cap/` (base only) and in
...
└── package.json      # @sap/cds ^10, "abap2UI5": "file:../core", Node ≥ 22
```

The z2ui5 runtime itself is not part of this project — it comes from the
[core package](../core/) (`require("abap2UI5/engine")`), which `srv/server.js`
...
[discovery API](../core/srv/app/samples/README.md#discovery-api). The bundled
samples and the framework classes live in the core package (`../core`), which
is owned by the sync pipeline.
