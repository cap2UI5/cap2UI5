...
|---|---|---|
| [**`builder-abap2UI5-js/`**](builder-abap2UI5-js/) | **abap2UI5 ported to JavaScript**: the ABAP→JS transpiler and sync scripts that generate the platform-neutral core package, plus the platform adapters that consume it. | [builder-abap2UI5-js/README.md](builder-abap2UI5-js/README.md) |
| ├─ [`builder-abap2UI5-js/core/`](builder-abap2UI5-js/core/) | the generated npm package `abap2UI5` — engine, transpiled classes, webapp, samples. Everything below consumes it via a `file:` link. | [core/README.md](builder-abap2UI5-js/core/README.md) |
| └─ [`builder-abap2UI5-js/adapters/`](builder-abap2UI5-js/adapters/) | the four platform adapters: [`cap`](builder-abap2UI5-js/adapters/cap/) (thinnest CAP wrapper), [`node`](builder-abap2UI5-js/adapters/node/) (bare `node:http`), [`express`](builder-abap2UI5-js/adapters/express/) (middleware), [`web`](builder-abap2UI5-js/adapters/web/) (serverless browser bundle). | each adapter's README |
| [**`builder-cap2UI5/`**](builder-cap2UI5/) | **The CAP app build project**: generates the full CAP app from its hand-written `src/` plus the core package. | [builder-cap2UI5/README.md](builder-cap2UI5/README.md) |
...
> [!NOTE]
> **`builder-abap2UI5-js/core/` and `cap2UI5/` are generated build artifacts in this
> repository** — every build wipes and rewrites them from `builder-abap2UI5-js/src/`
> resp. `builder-cap2UI5/src/` + the transpiled sources. If you are contributing
> here, edit the sources in the `src/` folders, not the published ones, and
> re-run `npm run build_core` (builder-abap2UI5-js) / `npm run build_cap` (builder-cap2UI5).
> (Consumers who copy `builder-abap2UI5-js/core/` + `cap2UI5/` out and use them
...
[builder-abap2UI5-js/](builder-abap2UI5-js/) mirrors the upstream abap2UI5 ABAP sources, transpiles
them to JavaScript, **assembles** the core package by overlaying the generated
trees on the hand-written source in `builder-abap2UI5-js/src/`, and **publishes** it 1:1
into `builder-abap2UI5-js/core/`; [builder-cap2UI5/](builder-cap2UI5/) then derives the full CAP
...
in [builder-abap2UI5-js/README.md](builder-abap2UI5-js/README.md).
