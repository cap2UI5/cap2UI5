...
// TODO(abap2js): unresolved reference z2ui5_cl_a2ui5_json_fltr — add require manually
...
class z2ui5_cl_core_handler {
  ...
  request_parse_body({ val } = {}) {
    ...
    const lv_root = ((lo_ajson.exists(`/value`) === true || lo_ajson.exists(`/value`) === `X`) ? `/value` : null);
    const lv_model_edit_name = `/${z2ui5_if_core_types.cs_ui5.two_way_model}`;
    result.o_model = z2ui5_cl_ajson.create_empty();
    const lo_model = lo_ajson.slice(lv_root + lv_model_edit_name);
    result.o_model.set({ iv_path: lv_model_edit_name, iv_val: lo_model });
    lo_ajson = lo_ajson.slice(lv_root + `/S_FRONT`);
    // TODO(abap2js): request_parse_event_args( EXPORTING io_front = lo_ajson IMPORTING ev_check_override = DATA(lv_check_arg_object) et_event_arg = DATA(lt_event_arg) ).
    // TODO(abap2js): lo_ajson->to_abap( EXPORTING iv_corresponding = abap_true IMPORTING ev_container = result-s_front ).
    if ((lv_check_arg_object === true || lv_check_arg_object === `X`)) {
      result.s_front.t_event_arg = z2ui5_cl_util.abap_copy(lt_event_arg);
    }
  ...
  request_parse_event_args({ io_front, ev_check_override, et_event_arg } = {}) {
    let lv_arg_path;
    et_event_arg = null;
    ev_check_override = null;
    let lv_arg_index = 1;
    for (let sy_index = 1; ; sy_index++) {
      lv_arg_path = `/T_EVENT_ARG/${lv_arg_index}`;
      switch (io_front.get_node_type(lv_arg_path)) {
        case ``:
          break;
          break;
        case z2ui5_if_ajson_types.node_type.object:
        case z2ui5_if_ajson_types.node_type.array:
          ev_check_override = true;
          et_event_arg.push(io_front.slice(lv_arg_path).stringify());
          break;
        case z2ui5_if_ajson_types.node_type.boolean:
          et_event_arg.push((io_front.get_boolean(lv_arg_path)));
          break;
        default:
          et_event_arg.push(io_front.get_string(lv_arg_path));
          break;
      }
      lv_arg_index = lv_arg_index + 1;
    }
    if ((ev_check_override === true || ev_check_override === `X`)) {
  ...
  request_app_start({ iv_search, io_comp_data } = {}) {
    ...
    try {
      if (io_comp_data != null) {
        result = z2ui5_cl_a2ui5_context.c_trim_upper(io_comp_data.get(`/startupParameters/app_start/1`));
      }
    } catch (error) {
    }
    if (result) {
    ...
    result = z2ui5_cl_a2ui5_context.c_trim_upper(z2ui5_cl_a2ui5_context.url_param_get({ val: `app_start`, url: iv_search }));
    return result;
  }
  ...
  request_app_start_draft({ iv_hash } = {}) {
    ...
    try {
      ...
      result = z2ui5_cl_a2ui5_context.c_trim_upper(z2ui5_cl_a2ui5_context.url_param_get({ val: `z2ui5-xapp-state`, url: lv_hash }));
    } catch (error) {
    }
    return result;
  }
  ...
  main_loop() {
    ...
    for (let sy_index = 1; ; sy_index++) {
      ...
      if (lv_dispatch_count >= this.mv_dispatch_limit) {
        throw new z2ui5_cx_a2ui5_error({ val: `Dispatch limit of ${this.mv_dispatch_limit} app navigations in one request reached - check for an endless nav_app_call/nav_app_leave loop in main( )` });
      }
    }
  }
  ...
  main_end() {
    this.ms_response = { s_front: { params: this.mo_action.ms_next.s_set, id: this.mo_action.mo_app.ms_draft.id, app: z2ui5_cl_a2ui5_context.rtti_get_classname_by_ref(this.mo_action.mo_app.mo_app) } };
  ...
  main_process() {
    ...
    try {
      ...
      lx2 = new z2ui5_cx_a2ui5_error({ val: `UNCAUGHT EXCEPTION - Please Restart App:`, previous: lx });
