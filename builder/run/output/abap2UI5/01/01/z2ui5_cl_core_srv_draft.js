...
class z2ui5_cl_core_srv_draft {
  ...
  cleanup() {
    ...
    const lv_n_hours_ago = z2ui5_cl_a2ui5_context.time_subtract_seconds({ time: z2ui5_cl_a2ui5_context.time_get_timestampl(), seconds: z2ui5_cl_core_srv_draft.c_seconds_per_hour * lv_exp_time_in_hours });
  ...
  create({ draft, model_xml } = {}) {
    ...
    const ls_db = { id: draft.id, id_prev: draft.id_prev, id_prev_app: draft.id_prev_app, id_prev_app_stack: draft.id_prev_app_stack, timestampl: z2ui5_cl_a2ui5_context.time_get_timestampl(), data: model_xml };
    z2ui5_port.db({ op: `modify`, table: `z2ui5_t_01`, row: ls_db });
    sy_subrc = z2ui5_port.sy_subrc;
    if (sy_subrc !== 0) {
      throw new z2ui5_cx_a2ui5_error({ val: `CREATE_OF_DRAFT_ENTRY_ON_DATABASE_FAILED` });
    }
    z2ui5_port.db({ op: `commit` });
  }
  ...
  read({ id, check_load_app = true } = {}) {
    ...
    if (sy_subrc !== 0) {
      throw new z2ui5_cx_a2ui5_error({ val: `NO_DRAFT_ENTRY_OF_PREVIOUS_REQUEST_FOUND` });
    }
    return result;
  }
