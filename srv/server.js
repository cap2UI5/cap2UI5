...
cds.on("bootstrap", (app) => {
  ...
  // on the way back out, stored in a LargeString column. Until now the only
  // limit was express's 100kb default, which applied by accident rather than
  // by decision and would silently change if anything reconfigured the parser.
  // Set it explicitly, and generously enough for a legitimately large view
  // model (Z2UI5_MAX_BODY overrides; a bad value falls back to the default
  ...
  // The GET route below applies the framework's t_security_header when it
  // renders the shell; the POST roundtrip and the OData entities answered
  // nothing at all, so a response carrying application state came back without
  // nosniff, without a frame guard and cacheable by any intermediary.
