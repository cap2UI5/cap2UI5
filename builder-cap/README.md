# builder-cap/ — the CAP app build project

This project **generates the deployable CAP app** [`cap2UI5/`](../cap2UI5/)
from the hand-maintained source in [`base/`](base/) plus the webapp shipped by
the published [core package](../transpile/core/).

> [!IMPORTANT]
> **`cap2UI5/` is a build artifact — do not hand-edit it.** Every build wipes
> and rewrites it. The hand-written source lives in [`base/`](base/); edit
> there and re-run the build. (Your own apps go in `base/srv/app/` or an
...
cd ../transpile && npm install && npm run build_core
...
| Step | What it does |
|---|---|
| `npm run assemble` | `base/` → `run/output/cap2UI5` (verbatim), then overlay `../transpile/core/app/z2ui5/webapp` → `app/z2ui5/webapp` (the copy served by CDS statics and zipped by the mta html5 module — taken from the published core, so the two cannot drift) |
...
dependency `abap2UI5` (`file:../transpile/core`). The only transformation in
the build: `base/` is one directory deeper than the published `cap2UI5/`, so
the copy rewrites the relative dependency path
(`file:../../transpile/core` → `file:../transpile/core`) in `package.json` and
`package-lock.json`.

## `base/` — the hand-written source

`base/` is the **source of truth** for the CAP app: the skeleton (`server.js`,
...
`base/` is itself a fully functional minimal CAP project — the starting point
...
See the [minimal base section](base/README.md#the-minimal-base) in the base
README for details.
