// Builds the static cap2UI5 web site into web/dist/:
//
...
//   3. copy the unchanged UI5 frontend (cap2UI5/app/z2ui5/webapp)
//   4. patch dist/index.html so the bundle loads BEFORE the UI5 bootstrap
//
// The result is fully static — open it from any web server (GitHub Pages,
// `npm run serve`, ...); every z2ui5 roundtrip is answered in-process by
// the bundled backend.
...
const CAP_DIR = path.join(WEB_DIR, "..", "cap2UI5");
...
const stub = (name) => path.join(WEB_DIR, "stubs", name);

const buildOptions = {
  entryPoints: [path.join(WEB_DIR, "entry.mjs")],
