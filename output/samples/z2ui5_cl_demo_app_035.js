...
class z2ui5_cl_demo_app_035 extends z2ui5_if_app {
  ...
  view_display() {
    ...
    const temp = page.simple_form({ title: `File`, editable: true })
      ...
      .label(`Option`);
    this.lt_types = {};
    this.lt_types = (() => { const __out = []; for (const row of (z2ui5_cl_util.source_get_file_types() ?? [])) { __out.push(...[{ n: this.shift_right(/* TODO(abap2js) */ shift_left(row)), v: this.shift_right(/* TODO(abap2js) */ shift_left(row)) }]); } return __out; })();
  ...
  read_abap() {
    let r_result = ``;
    let sy_subrc = 0;
    r_result = `METHOD SELECT_FILES.` + ` ` + ` ` + ` DATA: LV_RET_CODE TYPE I,` + ` ` + ` LV_USR_AXN TYPE I.` + ` ` + ` ` + ` CL_GUI_FRONTEND_SERVICES=>FILE_OPEN_DIALOG(` + ` ` + ` EXPORTING` + ` ` + ` WINDOW_TITLE = 'Select file'` + ` ` + ` MULTISELECTION = 'X'` + ` ` + ` CHANGING` + ` ` + ` FILE_TABLE = ME->PT_FILETAB` + ` ` + ` RC = LV_RET_CODE` + ` ` + ` USER_ACTION = LV_USR_AXN` + ` ` + ` EXCEPTIONS` + ` ` + ` FILE_OPEN_DIALOG_FAILED = 1` + ` ` + ` CNTL_ERROR = 2` + ` ` + ` ERROR_NO_GUI = 3` + ` ` + ` NOT_SUPPORTED_BY_GUI = 4` + ` ` + ` OTHERS = 5` + ` ` + ` ).` + ` ` + ` IF SY-SUBRC <> 0 OR` + ` ` + ` LV_USR_AXN = CL_GUI_FRONTEND_SERVICES=>ACTION_CANCEL.` + ` ` + ` RAISE EX_FILE_SEL_ERR.` + ` ` + ` ENDIF.` + ` ` + ` ` + ` ENDMETHOD. `;
    return r_result;
  }

  read_json() {
    let r_result = ``;
    r_result = `{` + ` ` + ` "quiz": {` + ` ` + ` "sport": {` + ` ` + ` "q1": {` + ` ` + ` "test" : false,` + ` ` + ` "question": "Which one is correct team name in NBA?",` + ` ` + ` "options": [` + ` ` + ` "New York Bulls",` + ` ` + ` "Los Angeles Kings",` + ` ` + ` "Golden State Warriros",` + ` ` + ` "Huston Rocket"` + ` ` + ` ],` + ` ` + ` "answer": "Huston Rocket"` + ` ` + ` }` + ` ` + ` },` + ` ` + ` "maths": {` + ` ` + ` "q1": {` + ` ` + ` "question": "5 + 7 = ?",` + ` ` + ` "options": [` + ` ` + ` "10",` + ` ` + ` "11",` + ` ` + ` "12",` + ` ` + ` "13"` + ` ` + ` ],` + ` ` + ` "answer": "12"` + ` ` + ` },` + ` ` + ` "q2": {` + ` ` + ` "question": true,` + ` ` + ` "options": [` + ` ` + ` "1",` + ` ` + ` "2",` + ` ` + ` "3",` + ` ` + ` "4"` + ` ` + ` ],` + ` ` + ` "answer": 487829` + ` ` + ` }` + ` ` + ` }` + ` ` + ` }` + ` ` + `}`;
    return r_result;
  }

  read_js() {
    let r_result = ``;
    r_result = `function showAlert() {` + ` ` + ` alert("Alert from JS file");` + ` ` + `}` + ` ` + ` ` + `function updateHeading() {` + ` ` + ` document.getElementById('heading')
      .innerHTML = 'Heading changed with JS';` + ` ` + `}`;
    return r_result;
  }

  read_yaml() {
    let r_result = ``;
    r_result = `# Employee records` + ` ` + `- martin:` + ` ` + ` name: Martin Developer` + ` ` + ` job: Developer` + ` ` + ` skills:` + ` ` + ` - python` + ` ` + ` - perl` + ` ` + ` - pascal` + ` ` + `- tabitha:` + ` ` + ` name: Tabitha Bitumen` + ` ` + ` job: Developer` + ` ` + ` skills:` + ` ` + ` - lisp` + ` ` + ` - fortran` + ` ` + ` - erlang`;
    return r_result;
  }

  read_text() {
    let r_result = ``;
    r_result = `TXT test file` + ` ` + `Purpose: Provide example of this file type` + ` ` + `Document file type: TXT` + ` ` + `Version: 1.0` + ` ` + `Remark:` + ` ` + ` ` + `Example content:` + ` ` + `The names "John Doe" for males, "Jane Doe" or "Jane Roe" for females, or "Jonnie Doe" and "Janie Doe" for children, or just ` + ` "Doe" non-gender-specifically are used as placeholder names for a party whose true identity is unknown or must ` + `be withheld in a legal action, case, or discussion. The names are also used to refer to a corpse or hospital patient whose ` + `identity is unknown. This practice is widely used in the United States and Canada, but is rarely used in other ` + `English-speaking countries including the United Kingdom itself, from where the use of "John Doe" in a legal context ` + `originates. The names Joe Bloggs or John Smith are used in the UK instead, as well as in Australia and New Zealand.` + ` ` + ` ` + `John Doe is sometimes used to refer to a typical male in other contexts as well, in a similar manner to John Q. Public,` + ` known in Great Britain as Joe Public, John Smith or Joe Bloggs. For example, the first name listed on a form is often ` + `John Doe, along with a fictional address or other fictional information to provide an example of how to fill in the form` + `. The name is also used frequently in popular culture, for example in the Frank Capra film Meet John Doe. John Doe was ` + `also the name of a 2002 American television series.`;
    return r_result;
  }
}
