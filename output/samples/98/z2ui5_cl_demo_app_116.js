...
class z2ui5_cl_demo_app_116 extends z2ui5_if_app {
  ...
  on_init() {
    ...
    this.gv_user = z2ui5_cl_util.abap_copy(sy_uname);
    this.gv_date = z2ui5_cl_util.abap_copy(sy_datum);
  }
  ...
  async main(client) {
    ...
    if (client.check_on_init()) {
      this.on_init();
      lv_save_state_js = `function saveState() {debugger;` + ` ` + ` var treeTable = z2ui5.oView.byId("treeTable");` + ` ` + ` z2ui5.treeState = treeTable.getBinding('rows')
        .getCurrentTreeState();` + ` ` + ` }; `;
      lv_reset_state_js = `function setState() {debugger;` + ` ` + ` var treeTable = z2ui5.oView.byId("treeTable");` + ` ` + ` if( z2ui5.treeState == undefined ) {` + ` ` + ` z2ui5.treeState = treeTable.getBinding('rows')
        .getCurrentTreeState();` + ` ` + ` } else {` + ` ` + ` treeTable.getBinding("rows")
        .setTreeState(z2ui5.treeState);` + ` ` + ` treeTable.getBinding("rows")
        .refresh();` + ` ` + ` z2ui5.treeState = treeTable.getBinding('rows')
        .getCurrentTreeState();` + ` ` + ` };` + ` ` + `};`;
    ...
    switch (client.get().EVENT) {
      ...
      case `POPOVER`:
        lt_event_arg = z2ui5_cl_util.struct_lower_keys(client.get().T_EVENT_ARG);
        lv_open_by_id = z2ui5_cl_util.abap_copy(lt_event_arg[(1) - 1]);
