...
class z2ui5_cl_demo_app_090 extends z2ui5_if_app {
  ...
  async main(client) {
    ...
    } else if (!(this.check_view_loaded === true || this.check_view_loaded === `X`)) {
  ...
  get_custom_js() {
    let result = ``;
    result = `z2ui5.setInitialData = () => {` + ` ` + ` var oView = z2ui5.oView` + ` ` + ` var oSelectionPanel = oView.byId("columnsPanel");` + ` ` + ` var oSortPanel = oView.byId("sortPanel");` + ` ` + ` var oGroupPanel = oView.byId("groupPanel");` + ` ` + ` oSelectionPanel.setP13nData(oView.getModel().oData.XX.MT_COLUMNS_P13N);` + ` ` + ` oSortPanel.setP13nData(oView.getModel().oData.XX.MT_SORT_P13N);` + ` ` + ` oGroupPanel.setP13nData(oView.getModel().oData.XX.MT_GROUPS_P13N);` + ` ` + ` var oPopup = oView.byId("p13nPopup");` + ` ` + ` oPopup.open();` + ` ` + `};` + ` ` + `z2ui5.updateData = (oReason) => {` + ` ` + ` if( oReason === "Ok" ) {` + ` ` + ` var oView = z2ui5.oView` + ` ` + ` var oSelectionPanel = oView.byId("columnsPanel");` + ` ` + ` var oSortPanel = oView.byId("sortPanel");` + ` ` + ` var oGroupPanel = oView.byId("groupPanel");` + ` ` + ` oView.getModel()
      .oData.XX.MT_COLUMNS_P13N = oSelectionPanel.getP13nData();` + ` ` + ` oView.getModel()
      .oData.XX.MT_SORT_P13N = oSortPanel.getP13nData();` + ` ` + ` oView.getModel()
