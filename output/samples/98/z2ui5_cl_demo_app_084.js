...
class z2ui5_cl_demo_app_084 extends z2ui5_if_app {
  ...
  async main(client) {
    ...
    if (client.check_on_init()) {
      lv_script = `` + ` ` + `function setInputFIlter(){` + ` ` + ` var inp = z2ui5.oView.byId('suggInput');` + ` ` + ` inp.setFilterFunction(function(sValue, oItem){` + ` ` + ` var aSplit = sValue.split(" ");` + ` ` + ` if (aSplit.length > 0) {` + ` ` + ` var sTermNew = aSplit.slice(-1)[0];` + ` ` + ` sTermNew.trim();` + ` ` + ` if (sTermNew) {` + ` ` + ` return oItem.getText().match(new RegExp(sTermNew, "i"));` + ` ` + ` }` + ` ` + ` }` + ` ` + ` });` + ` ` + `}`;
  ...
  view_display() {
    ...
    const page = view.shell()
      .page({ showheader: (!((this.client.get().CHECK_LAUNCHPAD_ACTIVE) === true || (this.client.get().CHECK_LAUNCHPAD_ACTIVE) === `X`)), title: `abap2UI5 - Selection-Screen Example`, navbuttonpress: this.client._event_nav_app_leave(), shownavbutton: this.client.check_app_prev_stack() });
