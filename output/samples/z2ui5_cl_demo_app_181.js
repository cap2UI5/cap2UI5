...
class z2ui5_cl_demo_app_181 extends z2ui5_if_app {
  ...
  view_display() {
    ...
    const card_1 = page.card({ width: `300px`, class: `sapUiMediumMargin` })
      ...
      .combobox({ width: `120px`, placeholder: `From City`, items: `{path:'` + this.client._bind({ val: this.mt_cities, path: true }) + `', sorter: { path: 'TEXT' } }` })
      .get()
      .item({ key: `{KEY}`, text: `{TEXT}` })
      .get_parent()
      .combobox({ width: `120px`, placeholder: `To City`, items: `{path:'` + this.client._bind({ val: this.mt_cities, path: true }) + `', sorter: { path: 'TEXT' } }` })
      .get()
      .item({ key: `{KEY}`, text: `{TEXT}` })
      .get_parent()
      .get_parent()
