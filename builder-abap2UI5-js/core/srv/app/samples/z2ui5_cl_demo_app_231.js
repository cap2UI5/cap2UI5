...
class z2ui5_cl_demo_app_231 extends z2ui5_if_app {
  ...
  view_display({ client } = {}) {
    ...
    vbox.label({ text: `DateRangeSelection with OK button in the footer and with shortcut for today:"`, labelfor: `DRS3` })
      .date_range_selection({ id: `DRS3`, showfooter: true, change: client._event(`handleChange`, [`DRS3`]), datevalue: `{= Helper.DateCreateObject($` + client._bind(this.drs3.start, { name: `drs3-start` }) + `) }`, seconddatevalue: `{= Helper.DateCreateObject($` + client._bind(this.drs3.end, { name: `drs3-end` }) + `) }` });
