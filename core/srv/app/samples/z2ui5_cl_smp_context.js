...
class z2ui5_cl_smp_context {
  ...
  static app_get_url({ classname, origin, pathname, search, hash = `` } = {}) {
    ...
    let lv_content;
    let lv_off;
    const lt_param = z2ui5_cl_smp_context.url_param_get_tab({ i_val: search });
    ...
    result = `${origin}${pathname}?` + z2ui5_cl_smp_context.url_param_create_url({ t_params: lt_param }) + lv_hash;
    return result;
  }
  ...
  static c_trim({ val } = {}) {
    ...
    result = (result.endsWith(z2ui5_cl_smp_context.cv_char_util_horizontal_tab) ? result.slice(0, -(z2ui5_cl_smp_context.cv_char_util_horizontal_tab).length) : result);
  ...
  static filter_get_range_by_token({ val } = {}) {
    ...
    switch (String(lv_value).substr(0, 1)) {
      case `=`:
  ...
  static itab_get_itab_by_csv({ val } = {}) {
    ...
    _fs$fs_tab = null;
    sy_subrc = 0;
    for (let _i = lt_rows.length - 1; _i >= 0; _i--) { const row = lt_rows[_i]; if (z2ui5_cl_util.abap_is_initial(row.table_line)) lt_rows.splice(_i, 1); }
    sy_tabix = 0;
  ...
  static url_param_get_tab({ i_val } = {}) {
    ...
    for (const lr_param of lt_param) {
      ...
      rt_params.push(z2ui5_cl_util.abap_copy({ n: z2ui5_cl_smp_context.c_trim_lower({ val: lv_name }), v: lv_value }));
    }
    return rt_params;
  }
  ...
  static rtti_get_t_attri_by_table_name({ table_name } = {}) {
    ...
    let lo_tab;
    let lt_attri;
    if (z2ui5_cl_util.abap_is_initial(table_name)) {
  ...
  static rtti_get_t_attri_on_prem({ tabname } = {}) {
    ...
    try {
      ...
      if (z2ui5_cl_util.abap_is_initial(tabname)) {
  ...
  static rtti_get_t_dfies_by_table_name({ table_name } = {}) {
    let result = [];
    if (!z2ui5_cl_util.abap_is_initial(z2ui5_cl_smp_context.context_check_abap_cloud())) {
  ...
  static msg_get_rap_element({ val } = {}) {
    ...
    for (const ls_attri of lt_attri) {
      ...
      if (!(String(ls_attri.name).substr(0, 9) === `%ELEMENT-`)) continue;
  ...
  static msg_get_rap_action({ val } = {}) {
    ...
    for (const ls_attri of lt_attri) {
      ...
      if (!(String(ls_attri.name).substr(0, 12) === `%OP-%ACTION-`)) continue;
      _fs$fs_flag = ((_o, _c) => { if (_o == null) return null; const _k = typeof _c === "number" ? Object.keys(_o)[_c - 1] : String(_c).toLowerCase(); return _k != null && _k in _o ? { o: _o, k: _k } : null; })(val, ls_attri.name);
      fs_flag = _fs$fs_flag ? _fs$fs_flag.o[_fs$fs_flag.k] : null;
      sy_subrc = _fs$fs_flag ? 0 : 4;
      if (!(sy_subrc === 0)) continue;
