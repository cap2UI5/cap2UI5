...
`;
    result = result + `          this.displayEditor(oModel, toJson(jsonSources[selItem]()), "json");` + `
` + `          return;` + `
` + `        }` + `
` + `` + `
...
`;
    result = result + `        // fragment each time raced the close animation on older UI5 (1.71):` + `
