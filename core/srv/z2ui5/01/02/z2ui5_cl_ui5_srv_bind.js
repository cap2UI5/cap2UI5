...
/**
 * z2ui5_cl_ui5_srv_bind — JS port of abap2UI5 z2ui5_cl_ui5_srv_bind.
 *
...
class z2ui5_cl_ui5_srv_bind {
  ...
  static main_one_way(client, val, opts = {}) {
    return z2ui5_cl_ui5_srv_bind._main(client, val, opts, z2ui5_if_ui5_types.cs_bind_type.one_way, `__bind`);
  }
  ...
  static main_two_way(client, val, opts = {}) {
    return z2ui5_cl_ui5_srv_bind._main(client, val, opts, z2ui5_if_ui5_types.cs_bind_type.two_way, `__edit`);
  }
  ...
  static _main(client, val, opts, type, prefix) {
    ({ val, opts } = z2ui5_cl_ui5_srv_bind._unpack_named_args(val, opts));
    if (opts.path_only) opts = { ...opts, path: true };

    const modelPrefix =
      type === z2ui5_if_ui5_types.cs_bind_type.two_way
        ? `/${z2ui5_if_ui5_types.cs_ui5.two_way_model}`
        : ``;
    ...
    } else if (z2ui5_cl_ui5_srv_bind._is_filled_tab(opts.tab)) {
      full = `${modelPrefix}/${z2ui5_cl_ui5_srv_bind._path_tab_cell(client, val, type, prefix, opts)}`;
    } else if (opts.name) {
      const byName = z2ui5_cl_ui5_srv_bind._path_by_name(client, val, type, opts);
      if (byName) full = `${modelPrefix}/${byName}`;
    }
    if (full === null) {
      const prop = z2ui5_cl_ui5_srv_bind._find_or_create(client, val, type, prefix, opts);
      full = `${modelPrefix}/${prop.toUpperCase()}`;
    }

    if (opts.path === true) return full;
    return z2ui5_cl_ui5_srv_bind._build_expr(full, opts);
  }
  ...
  static _unpack_named_args(val, opts = {}) {
    if (
      ...
      && Object.keys(val).every((k) => z2ui5_cl_ui5_srv_bind._NAMED_ARG_KEYS.has(k))
    ) {
  ...
  static _path_tab_cell(client, val, type, prefix, opts) {
    const tab = opts.tab;
    const tabProp = z2ui5_cl_ui5_srv_bind._find_or_create(client, tab, type, prefix, {});
  ...
  static _path_by_name(client, val, type, opts) {
    ...
    for (const seg of segs) {
      ...
      const key = z2ui5_cl_ui5_srv_bind._match_key(cur, seg);
    ...
    if (!existing) {
      z2ui5_cl_ui5_srv_bind.check_raise_new(opts);
      client.aBind.push(z2ui5_cl_ui5_srv_bind._entry({ name: rootProp, val: client.oApp[rootProp], type, opts }));
    }
  ...
  static main_local(client, val) {
    ...
    client.aBind.push({ name, val, type: z2ui5_if_ui5_types.cs_bind_type.one_way });
    return `{/${name}}`;
  }
  ...
  constructor(app) {
    this.mo_app    = app;             // the owning z2ui5_cl_ui5_app_cont
  ...
  main_cell(a, b, c, d) {
    ...
    const lo_bind = new z2ui5_cl_ui5_srv_bind(this.mo_app);
  ...
  _get_client_name() {
    ...
    const prefix = this.mv_type === z2ui5_if_ui5_types.cs_bind_type.two_way
      ? `/${z2ui5_if_ui5_types.cs_ui5.two_way_model}`
      : ``;
    return `${prefix}/${n}`;
  }
  ...
  _main_legacy(client, val, type, config = {}) {
    ...
    let result =
      type === z2ui5_if_ui5_types.cs_bind_type.two_way
        ? z2ui5_cl_ui5_srv_bind.main_two_way(client, val, opts)
        : z2ui5_cl_ui5_srv_bind.main_one_way(client, val, opts);

    // abap raises if the resolved name happens to be the reserved XX prefix.
    const xxPath = `/${z2ui5_if_ui5_types.cs_ui5.two_way_model}`;
  ...
  static get_client_name(name, type) {
    ...
    const prefix = type === z2ui5_if_ui5_types.cs_bind_type.two_way
      ? `/${z2ui5_if_ui5_types.cs_ui5.two_way_model}`
      : ``;
    return `${prefix}/${n}`;
  }
   ...
   *
   * Validation matches abap z2ui5_cl_ui5_srv_bind:
  ...
  static _find_or_create(client, val, type, prefix, opts = {}) {
    ...
    for (const prop in client.oApp) {
      ...
      if (!z2ui5_cl_ui5_srv_bind._is_equal(client.oApp[prop], val)) continue;
      ...
      z2ui5_cl_ui5_srv_bind.check_raise_new(opts);
      client.aBind.push(z2ui5_cl_ui5_srv_bind._entry({ name: prop, val, type, opts }));
      return prop;
    }
    ...
    client.aBind.push(z2ui5_cl_ui5_srv_bind._entry({ name, val, type, opts }));
    return name;
  }
...
module.exports = z2ui5_cl_ui5_srv_bind;
