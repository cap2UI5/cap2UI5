...
class z2ui5_cl_demo_app_288 extends z2ui5_if_app {
  ...
  view_display({ client } = {}) {
    ...
    const page_02 = page_01.page({ showheader: false, class: `sapUiContentPadding` })
      ...
      .select({ enabled: client._bind_edit(this.enabled), editable: client._bind_edit(this.editable), forceselection: false, selectedkey: client._bind_edit(this.selected_product2), items: client._bind(this.lt_product_collection2) })
      .item({ key: `{PRODUCT_ID}`, text: `{NAME}` })
      .get_parent()
      .vbox()
      ...
      .toolbar()
      .toolbar_spacer()
      .select({ forceselection: false, selectedkey: client._bind_edit(this.selected_product3), type: `IconOnly`, icon: `sap-icon://filter`, autoadjustwidth: true, items: client._bind(this.lt_product_collection3) })
      .item({ key: `{PRODUCT_ID}`, text: `{NAME}` });
