...
class z2ui5_cl_demo_app_339 extends z2ui5_if_app {
  ...
  on_event({ client } = {}) {
    switch (client.get().EVENT) {
      case `SELECTION_CHANGE`:
        client.nav_app_call(z2ui5_cl_demo_app_340.factory({ io_table: this.mt_table, io_layout: this.mo_layout }));
        break;
      case `BACK`:
  ...
  view_display({ client } = {}) {
    ...
    this.mo_layout = z2ui5_cl_demo_app_333.factory({ i_data: this.mt_table, vis_cols: 5 });
  ...
  get_data() {
    ...
    try {
      ...
      // TODO(abap2js): SELECT * FROM (mv_table) INTO CORRESPONDING FIELDS OF TABLE @<table> UP TO 3 ROWS.
      fs_table.sort();
    } catch (error) {
    }
