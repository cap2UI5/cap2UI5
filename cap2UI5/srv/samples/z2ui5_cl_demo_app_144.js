...
class z2ui5_cl_demo_app_144 extends z2ui5_if_app {
  ...
  set_view() {
    ...
    const tab = page.table({ items: this.client._bind_edit(this.t_tab), mode: `MultiSelect` })
      ...
      .input(`{TITLE}`)
      .input(`{VALUE}`);
    page.input(this.client._bind_edit({ val: this.t_tab[(1) - 1].title, tab: this.t_tab, tab_index: 1 }));
    page.input(this.client._bind_edit({ val: this.t_tab[(1) - 1].value, tab: this.t_tab, tab_index: 1 }));
    page.input(this.client._bind_edit({ val: this.t_tab[(2) - 1].title, tab: this.t_tab, tab_index: 2 }));
    page.input(this.client._bind_edit({ val: this.t_tab[(2) - 1].value, tab: this.t_tab, tab_index: 2 }));
