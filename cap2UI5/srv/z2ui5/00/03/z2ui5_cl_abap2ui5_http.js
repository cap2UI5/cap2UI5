...
class z2ui5_cl_abap2ui5_http {
  ...
  static client_create({ destination, url } = {}) {
    ...
    if (result != null) {
      throw new z2ui5_cx_abap2ui5_error({ val: `HTTP_CLIENT_CREATE_ERROR - check the destination/url configuration` });
    }
    return result;
  }
  ...
  static client_call({ method, body, destination, url } = {}) {
    ...
    let lv_message = ``;
    const lo_client = z2ui5_cl_abap2ui5_http.client_create({ destination, url });
    try {
      ...
      if (sy_subrc !== 0) {
        ...
        throw new z2ui5_cx_abap2ui5_error({ val: `HTTP_COMMUNICATION_ERROR - ${lv_message}` });
      }
  ...
  static factory({ server } = {}) {
    let result = null;
    result = new z2ui5_cl_abap2ui5_http();
  ...
  get_req_info() {
    ...
    result.t_params = z2ui5_cl_abap2ui5_context.url_param_get_tab(this.get_header_field({ val: `~request_uri` }));
    return result;
  }
}

module.exports = z2ui5_cl_abap2ui5_http;
