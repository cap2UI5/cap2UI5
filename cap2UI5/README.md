...
[`builder/base/`](https://github.com/cap2UI5/cap2UI5/tree/main/builder/base)
and published 1:1 into `cap2UI5/`. Everything else in the published project —
the full z2ui5 frontend under `app/z2ui5/webapp/`, the ~190 sample apps under
`srv/app/samples/` and the transpiled framework classes — is generated by the
sync pipeline **on top of** this base. All commands below work in both places:
in `builder/base/` (base only) and in `cap2UI5/` (base + generated overlays).
...
│   ├── z2ui5-service.js    # wires POST /rest/root/z2ui5 → z2ui5 runtime
│   ├── server.js           # GET bootstrap HTML + local UI5 runtime at /resources
│   ├── z2ui5/              # the z2ui5 runtime (ported abap2UI5 classes)
...
└── package.json      # @sap/cds ^10, Node ≥ 22, in-memory SQLite for dev
```
